...
from cornerdetection import cornerdetection_inputsignal, cornerdetection_outputsignal, \
    golay_filter_variable_window_size_input_signal, golay_filter_variable_window_size_output_signal, \
    windowsize_output_signal
...
for counter, df in enumerate([data_array[0]]):
    ...
    input_signal_troughs = np.unique(cornerdetection_inputsignal(input_signal, time, False))
    output_signal_troughs = np.unique(cornerdetection_outputsignal(output_signal, time, False))
    #
    for peak_index, peak_time in enumerate(output_signal_peaks):
        corresponding_trough_index = np.isclose(output_signal_troughs, peak_time, atol=0., rtol=0.00001).nonzero()[0]
        if len(corresponding_trough_index) == 0:
            continue
        else:
            for corindex in corresponding_trough_index:
                if corindex != 0:
                    if output_signal_peaks[peak_index-1] < output_signal_troughs[corindex - 1]:
                        # take peak before and trough after
                        newtime = peak_time + (output_signal_troughs[corindex - 1] - peak_time) / 2
                        output_signal_peaks[peak_index] = newtime
                    else:
                        #take trough before and peak after
                        newtime = peak_time + (output_signal_peaks[peak_index-1] - peak_time)/2
                        output_signal_troughs[corindex] = newtime
                else:
                    if output_signal_peaks[peak_index+1] < output_signal_troughs[corindex + 1]:
                        # take peak before and trough after
                        newtime = peak_time + (output_signal_peaks[peak_index+1] - peak_time)/2
    ...
    for peak_index, peak_time in enumerate(output_signal_peaks):
        ...
        window_set_start = [i for i, x in enumerate(output_signal_peaks) if x >= float(peak_time)-float(local_detection_window_size)/2][0]
        window_set_end =  [i for i, x in enumerate(output_signal_peaks) if x <= float(peak_time)+float(local_detection_window_size)/2][-1]
        if window_set_start != window_set_end:
            for close_doubles_index, close_doubles in enumerate(output_signal_peaks[window_set_start:window_set_end]):
                ...
                if not np.isclose(peak_time, hightest_peak_time, atol=0., rtol=0.000001):
                    deletion_indices.append(peak_index)

    output_signal_peaks = np.delete(output_signal_peaks, deletion_indices)

    deletion_indices2 = []
    for trough_index, trough_time in enumerate(output_signal_troughs):
        windowsize_val = np.interp(trough_time, time,
        ...
        if windowsize_val > 23:
            ...
            local_detection_window_size = 0
        window_set_start = \
        [i for i, x in enumerate(output_signal_troughs) if x >= float(trough_time) - float(local_detection_window_size) / 2][0]
        window_set_end = \
        [i for i, x in enumerate(output_signal_troughs) if x <= float(trough_time) + float(local_detection_window_size) / 2][-1]
        if window_set_start != window_set_end:
            for close_doubles_index, close_doubles in enumerate(output_signal_troughs[window_set_start:window_set_end]):
                lowest_trough_index = np.argmin(
                    np.interp(output_signal_troughs[window_set_start:window_set_end], time, output_signal))
                lowest_trough_time = output_signal_troughs[window_set_start:window_set_end][lowest_trough_index]
                if not np.isclose(trough_time, lowest_trough_time, atol=0., rtol=0.000001):
                    deletion_indices2.append(trough_index)

    output_signal_troughs = np.delete(output_signal_troughs, deletion_indices2)


    switch_points = []
    output_signal_corners = np.sort(np.concatenate((output_signal_peaks, output_signal_troughs)))
    output_signal_corners_arg = np.argsort(np.concatenate((output_signal_peaks, output_signal_troughs)))
    ...
    print(output_signal_troughs, output_signal_peaks)
    print(input_signal_troughs, input_signal_peaks)
    ...
    figax[counter][1].vlines(x=input_signal_troughs, color='green', ymin=min(input_signal), ymax=max(input_signal))
    ...
    figax[counter][1].vlines(x=output_signal_troughs - np.ones((len(output_signal_troughs))) * 0.003, color='green', ymin=min(output_signal), ymax=max(output_signal))
    ...
    AB_dist = np.abs(input_signal_peaks[0] - input_signal_troughs[0])
    AB_stretch = [input_signal_troughs[0] + 0.25 * AB_dist, input_signal_peaks[0] - 0.25 * AB_dist]
